...
// NOTE: Defaults are RELATIVE paths on purpose. This works locally AND on any
// VM/domain without rebuilding the image, because the request/socket goes to
// whatever host+port served the page (nginx), which then proxies internally
// to the backend container over the docker-compose network (see nginx.conf).
// Only set NEXT_PUBLIC_BACKEND_HTTP/WS as a build ARG if backend is hosted
// on a completely different domain than the frontend.
const BACKEND_HTTP = process.env.NEXT_PUBLIC_BACKEND_HTTP || "";
const BACKEND_WS = process.env.NEXT_PUBLIC_BACKEND_WS || "/ws/drone-stream";
const BACKEND_DEVICE_WS = process.env.NEXT_PUBLIC_BACKEND_DEVICE_WS || "/ws/device-stream";
